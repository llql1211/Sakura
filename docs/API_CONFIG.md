...
本文教你在 Sakura 中配置大模型 API。适合第一次接触 API Key、Base URL 和模型名称的新手。
...
> Sakura 需要模型支持图像输入。屏幕观察、主动观察等功能会把截图发给模型；如果模型不支持多模态，相关功能会失败。新手优先选择 Gemini Flash 系列，不建议使用 DeepSeek 系列作为 Sakura 主模型。

---

## 使用中转站配置

下面以[哈基米 API](https://api.gemai.cc/register?aff=rwbQ) 站为例。你也可以使用其他 OpenAI 兼容服务商，只要能拿到 `Base URL`、`API Key` 和模型名称即可。如果不懂建议跟着教程来。

### 第一步：注册账号

进入[哈基米 API](https://api.gemai.cc/register?aff=rwbQ)

可以使用 QQ 邮箱注册。新用户有免费的额度来进行测试，之后觉得不错可以充值。
...
填写完成后，点击**检测模型**。如果服务商暴露了 `/models` 接口，Sakura 会读取可用模型列表，你可以从下拉框中选择模型。

再点击**测试 API**。弹出测试成功提示后，点击**Save** 保存。
